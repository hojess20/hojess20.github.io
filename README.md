## Hello!
My name is Jessica, and I am currently a Master's student in the Health Data Science program at the University of California San Francisco. 

### Projects
#### [Survival Differences Across PAM50 Subtypes in Breast Cancer](https://github.com/hojess20/ucsf-portfolio/tree/main/SurvivalAnalysis)
![Kaplan Meier Curve](images/kaplan-meier.png)

This project investigates the time-varying impact of PAM50 molecular subtypes on overall breast cancer survival. Using the METABRIC dataset of nearly 2,000 primary breast tumors, I developed a multivariable Cox proportional hazards model adjusting for age, tumor size, and histologic grade. To account for non-proportional hazards and non-linear risks, the analysis evaluated survival across three distinct time intervals (0–40, 40–80, and 80–170 months) and incorporated natural cubic splines to accurately model tumor size.

#### [Dementia Prediction](https://github.com/hojess20/hojess20.github.io/tree/main/DementiaPrediction)
This project explores predicting dementia diagnoses and identifying at-risk subgroups using clinical and neuropathological variables from the Aging, Dementia, and Traumatic Brain Injury (TBI) study. To analyze the data, I implemented both a logistic regression model and a random forest classifier to predict dementia status based on features like age, TBI history, Braak stage, and ApoE4 allele presence. Additionally, I applied a k-means clustering algorithm to uncover distinct natural subgroups within the population and evaluate their varying risk profiles.
---

[LinkedIn](https://www.linkedin.com/in/hojessica20/) || [GitHub](https://github.com/hojess20) || [Email](mailto:jho3@ucsf.edu)

